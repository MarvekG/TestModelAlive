...
export function initApp() {
  ...
  async function saveEndpoint() {
    ...
    const duplicate = endpoints.find((endpoint) => endpoint.type === request.type && endpoint.base_url === request.base_url && endpoint.name === request.name && endpoint.api_key === request.api_key);
    let overwrite = false;
    if (duplicate) {
    ...
    const duplicateName = endpoints.find((endpoint) => endpoint.type === request.type && endpoint.name === request.name && endpoint.id !== duplicate?.id);
    if (duplicateName) {
      await showAlert(t("saveEndpoint"), t("duplicateEndpointName"));
      return;
    }
    try {
