...
- The model test page offers "Restore Official Config", which removes top-level `llm-deepseek` and default-model overrides to restore DSH's built-in official configuration. "Delete This Provider" removes the current endpoint's `tma-<endpoint name>` provider, default-model reference, and associated API-key credential.
