...
- The model test page offers "Restore Official Config", which removes top-level `llm-deepseek` and default-model overrides to restore DSH's built-in official configuration. "Delete This Provider" removes only the current endpoint's `tma-<endpoint name>` provider and its default-model reference.
- The app reuses embedded context-window and maximum-output values. `llm-pi-ai` models also receive converted OpenCode reasoning levels; unknown models retain DSH defaults.
